...
│
├── src/
│   ├── JsonConverter.Core/          # UI 비의존 변환 로직 (재사용 · 테스트 대상)
│   │   ├── Models/                 #   데이터 모델 (테이블 정의, 설정 등)
│   │   └── Services/               #   다운로드 · TSV→JSON 변환 · 파일 저장 · 설정 저장
│   │
│   └── JsonConverter.App/           # WPF 프레젠테이션 계층 (net9.0-windows)
│       ├── App.xaml                #   앱 진입점
...
│
├── tests/
│   └── JsonConverter.Tests/         # JsonConverter.Core 단위 테스트
│
...
**계층 분리 원칙**: 변환·다운로드·저장 로직은 모두 `JsonConverter.Core`에 두고,
`JsonConverter.App`은 UI만 담당합니다. 테스트는 Core만 참조하므로 UI 없이 검증할 수 있습니다.
...
# 앱 실행
dotnet run --project src/JsonConverter.App

# 배포용 단일 파일 exe (publish/JsonConverter.exe, 약 60MB)
dotnet publish src/JsonConverter.App -o publish
```

퍼블리시 옵션(단일 파일·자체 포함·압축·네이티브 DLL 포함)은 `JsonConverter.App.csproj`에
