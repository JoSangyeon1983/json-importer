...
using JsonConverter.App.Infrastructure;
using JsonConverter.Core.Models;
using JsonConverter.Core.Services;

namespace JsonConverter.App.ViewModels;

/// <summary>
/// 메인 화면 뷰모델. 테이블 목록의 동적 추가/삭제, convert 실행, 로그, 설정 영구 저장을 담당합니다.
/// </summary>
public sealed class MainViewModel : ObservableObject
{
    ...
    public ICommand ConvertOneCommand { get; }
    public ICommand BrowseOutputCommand { get; }
    public ICommand OpenOutputFolderCommand { get; }

    public MainViewModel(SettingsStore settingsStore, ConvertService convertService)
    {
        ...
        ConvertAllCommand = new AsyncRelayCommand(_ => ConvertAllAsync(), _ => !IsBusy && Tables.Count > 0);
        ConvertOneCommand = new AsyncRelayCommand(p => ConvertOneAsync(p as TableEntryViewModel), _ => !IsBusy);
    ...
    private async Task ConvertAllAsync()
    {
        ...
        try
        {
            ...
            AppendLog($"=== 전체 Convert 시작 ({targets.Count} tables) ===");

            var success = 0;
            foreach (var vm in targets)
            {
                if (await ConvertEntryAsync(vm))
                    success++;
            }
    ...
    private async Task ConvertOneAsync(TableEntryViewModel? vm)
    {
        if (vm == null)
            return;
    ...
    private async Task<bool> ConvertEntryAsync(TableEntryViewModel vm)
    {
        vm.Status = "진행 중…";
        var result = await _convertService.ConvertAsync(vm.ToModel(), OutputRoot, AppendLog);
