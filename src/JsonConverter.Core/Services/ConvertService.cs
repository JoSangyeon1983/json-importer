...
/// <summary>
/// 하나의 테이블을 convert하는 전체 파이프라인을 조율합니다:
...
public sealed class ConvertService
{
    ...
    public ConvertService(ITextDownloader downloader, IJsonFileSaver saver)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(entry.Name))
            return ConvertResult.Failure("테이블 이름이 비어있습니다.");

        if (string.IsNullOrWhiteSpace(entry.Url))
            return ConvertResult.Failure($"URL이 비어있습니다: {entry.Name}");
        ...
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(tsv))
                return ConvertResult.Failure($"다운로드한 TSV가 비어있습니다: {entry.Name}");
            ...
            if (string.IsNullOrWhiteSpace(json) || json.Trim() == "{}")
                return ConvertResult.Failure($"변환된 JSON이 비어있습니다: {entry.Name}");
            ...
            return ConvertResult.Ok($"완료: {entry.Name} → {System.IO.Path.GetFileName(outputPath)}", outputPath, outputName);
        }
        catch (OperationCanceledException)
        {
            return ConvertResult.Failure($"취소됨: {entry.Name}");
        }
        catch (Exception ex)
        {
            return ConvertResult.Failure($"실패: {entry.Name} - {ex.Message}");
        }
    }
}
