...
public class ConvertServiceTests
{
    ...
    [Fact]
    public async Task Convert_WritesValidJsonFile()
    {
        var tsv = "id\tname\tcount\nA\tApple\t3\nB\tBanana\t5";
        var outputRoot = Path.Combine(Path.GetTempPath(), "JsonConverterTests", Guid.NewGuid().ToString("N"));

        try
        {
            var service = new ConvertService(new FakeDownloader(tsv), new JsonFileSaver());
            var entry = new DataTableEntry("Fruit", "http://example/tsv");

            var result = await service.ConvertAsync(entry, outputRoot);
    ...
    [Fact]
    public async Task Convert_UsesSheetName_ForOutputFileName()
    {
        var tsv = "id\tname\nA\tApple";
        var outputRoot = Path.Combine(Path.GetTempPath(), "JsonConverterTests", Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            var service = new ConvertService(new FakeDownloader(tsv, sheetName: "Localization"), new JsonFileSaver());
            var result = await service.ConvertAsync(new DataTableEntry("Table1", "http://example/tsv"), outputRoot);
    ...
    [Fact]
    public async Task Convert_EmptyUrl_Fails()
    {
        var service = new ConvertService(new FakeDownloader(""), new JsonFileSaver());
        var result = await service.ConvertAsync(new DataTableEntry("T", ""), "unused");
